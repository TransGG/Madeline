...
const DATABASE_PROMISE = MONGO_CLIENT_PROMISE.then((CLIENT) => CLIENT.db("Madeline"));
...
module.exports = {
    ...
    async setRoles(group, roles) {
        const DATABASE = await DATABASE_PROMISE;
        await DATABASE.collection("role_groups").updateOne({ key: group }, { $set: { roles } }, { upsert: true });
    }
};
